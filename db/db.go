...
主要用到的设计模式
	...
	5.迭代器模式;迭代器模式主要用在访问对象集合(复杂数据结构)的场景，能够向客户端隐藏集合的实现细节,提供易于使用的api
Db接口已经实现了查询接口,但我们还想提供全表查询接口,有随机和有序两种方式,并且支持客户端拓展遍历方式!
	6.工厂方法模式: 随机迭代器和有序迭代器本身属于同一类的产品,可以将其工厂抽象成接口,用户在调用处通过传入不同的工厂方法
来实现生产不同的迭代器接口实现实例,工厂方法将某一种类的东西交由一个工厂生产,添加新产品时只需要增加工厂即可
